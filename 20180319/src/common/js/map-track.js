...
class mapTrack{
    ...
    if (pType != "node"){
      ...
      if(typeof pointInfo.direction !== "undefined"){
        pointDirection = this.getDirection(pointInfo.direction);//方向指针号0~11
      }else{
        pointDirection = 0
      } 
      pointIcon = new BMap.Icon(require(`../images/node/pnode_${pointDirection}.png`), new BMap.Size(20, 20));
    }
	...
	// 按轨迹点画小车运行轨迹
	drawMapLine(map, trackData, stayPoints){
    ...
    let trackCount = trackData.length
	  ...
	  trackMarkers =[]
    // 增加停留点覆盖物
    for(let item of stayPoints){
      point = new BMap.Point(item.stay_point.longitude, item.stay_point.latitude);
      let pointMarker = this.createPointMarkers(map, item.stay_point, "stay");
    }
    // 增加起点终点方向点覆盖物
      ...
      // else if (trackData[i].StayTime >= 300 && trackData[i].LocateType!=2) //基站点不给停标记
	    // else if (trackData[i].stay_point == 1) //基站点不给停标记
	      // pointType = "stay";
	    else{
	      pointType = "node";
      }
    ...
    if (trackCount > 1) { 
      ...
      if (carMarker != null){
        ...
        for (var i = 1; i < trackCount - 1; i++){
          ...
          if (vm.trackData[i].stay_point == 1)
            trackMarkers[i].hide()
        }
      }else{
  ...
  getCarIcon(vm,index){
    ...
    if(typeof vm.trackData[index].direction !== "undefined"){
