...
<template>
    ...
    <el-dialog title="新增租车" :visible.sync="rentalDialog" model="right" width="30%" @close="onCloseRentalDialog">
      <el-form :model="formData" label-width="100px" :rules="formRule" ref="rentalForm">
        ...
        <el-form-item label="车牌号" prop="carId">
          <el-select v-model="formData.carId" placeholder="请选择租用车牌号" filterable @change="onLoadFeeByCarId">
            <el-option
        ...
        <el-form-item label="收费方式" prop="chargeType">
          <el-select v-model="formData.chargeType" placeholder="请选择收费方式" filterable @change="onLoadFeeByFeeType">
            <el-option
        ...
        <el-form-item :label="feeType===1?'起始里程':feeType===2?'起始油耗':'起始时间'" prop="fee" v-if="formData.chargeType">
          <el-input v-model="formData.fee" auto-complete="off"></el-input>
          <span>{{feeType===1?'元/公里':feeType===2?'元/升':''}}</span>
        </el-form-item>
      </el-form>
    ...
    <el-dialog
      ...
      <div v-if="returnCarInfo">
        ...
        <p>收费金额：{{returnCarInfo.unit_value}}</p>
      </div>
...
export default {
  ...
  data () {
    return {
      ...
      formData: { phone: '', identity: '', licence: '', carId:'', chargeType:'',fee: ''},
      formRule: {
        ...
        fee: [{ required: true, message: '请输入车辆起始值', trigger: 'blur' },{ validator: validator.checkPrice, trigger: 'blur' }]
      },
      carOption: [],
      chargeOption: [],
      feeType: 1,
  ...
  methods:{
    ...
    onLoadFeeByCarId(val){
      if(this.formData.carId && this.formData.chargeType){
        this._getCarOriginVal({vehicle_number: this.formData.carId,charge_scheme_id: this.formData.chargeType})
      }
    },
    onLoadFeeByFeeType(val){
      for(let item of this.chargeOption){
        if(item.charge_scheme_id == val){
          this.feeType = item.type
        }
      }
      if(this.formData.carId && this.formData.chargeType){
    ...
    _getCarOriginVal(params){
      getCarOriginVal(params).then(res=>{
        console.log(res)
        if(res.data.ret == 0){
          this.formData.fee = res.data.datas.unit_value
        }
      })
    },
    // 导出
