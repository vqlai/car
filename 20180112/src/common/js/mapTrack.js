...
class mapTrack{
	...
	// 按轨迹点画小车运行轨迹
	drawMapLine(map, trackData, trackCount){
	  let carMarker = null;
    ...
    // 显示起点与终点
    // for(let [index,item] of trackMarkers.entries()){
    //   if(index == 0 || index == trackMarkers.length-1){
    //     item.show();
    //   }else{
    //     item.hide();
    //   }
    ...
    if (trackCount > 1) { 
      ...
      function resetMkPoint(pIndex){
        // console.log(travelIsPlay)
        // console.log(travelIsPause)
        if (carIsPlay){
      ...
      trackPlayTimer = setTimeout(
        function () { resetMkPoint(1); }, trackSpeed
       // () => { this.resetMkPoint(map, 1, trackData, travelIsPlay, travelIsPause, trackSpeed); }, trackSpeed
      );
    }else{
      clearInterval(trackPlayTimer);
    }
  ...
  // resetMkPoint(map, pIndex, trackData, travelIsPlay, travelIsPause, trackSpeed){
  //   carCurrentIndex = pIndex
  //   let trackCount = trackData.length
  //   if (travelIsPlay){
  //     mPoint = trackMarkers[pIndex].getPosition();
  //     pointDirection = this.getDirection(trackData[pIndex].direction);//方向指针号0~11
  //     carIcon = new BMap.Icon(require(`../images/car/car_p_${pointDirection}.png`), new BMap.Size(36, 36));
  //     carMarker.setZIndex(100);
  //     carMarker.setIcon(carIcon);
  //     carMarker.setPosition(trackMarkers[pIndex].getPosition());
  ...
  //     movingPointIndex++;
  //   }else if (travelIsPause == false){//非暂停则重置
  //     movingPointIndex = 0;
  //   }

  //   if (movingPointIndex < trackCount){
  //     trackPlayTimer = setTimeout(
  //       () => { this.resetMkPoint(map,movingPointIndex,trackData, travelIsPlay, travelIsPause, trackSpeed); }, trackSpeed
  //     );
  //   }else{
  //     this.stopOrEndPlay(map, 1)
  //   }
  // }
