        ...
        <el-form :model="formData" label-width="100px" :rules="formRules" ref="carBindForm">
          ...
          <el-form-item label="品牌logo" prop="logo">
            <el-upload
              ...
              <div slot="tip" class="el-upload__tip">每次只能上传一张jpg/png文件，且不超过500kb</div>
            </el-upload>
          </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
          <el-button @click="onResetForm('carBindForm')">取 消</el-button>
          <el-button type="primary" @click="onSubmitForm('carBindForm')">保 存</el-button>
        </div>
      </el-dialog>
    </el-row>
    <el-table
...
export default {
  ...
  data () {
    return {
      ...
      formRules:{
        vechile_number: [{ required: true, message: '请输入车牌号', trigger: 'blur' }],
        tbox_id: [{ required: true, message: '请输入车载盒子序列号', trigger: 'blur' }],
        vehicle_brand: [{ required: true, message: '请输入车辆品牌', trigger: 'blur' }]
      },
      fileList: [], // {name:'',url:''}
      uploadUrl: '//10.0.0.211:9999/car/add', // '//10.0.0.211:9999/car/add'
