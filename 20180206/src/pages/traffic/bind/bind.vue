...
<template>
	<section class="bind">
	  <el-row type="flex" class="table-handle-box">
      <el-col :span="3">
        <el-input v-model="carId" placeholder="请输入车牌号"></el-input>
      </el-col>
  ...
  methods:{
    ...
    loadTableData(res, status, isUpload){
      ...
      if(code == 0){
        ...
        this._getBindCar({ page: this.currentPage, page_size: this.pageSize, vehicle_number: this.carId})
    ...
    onCurrentChange(page){
      if(!this.pageFlag){
        this.pageFlag = true
        this._getBindCar({page: page, page_size: this.pageSize, vehicle_number: this.carId})
      }
    },
    // 格式化时间
