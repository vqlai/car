  ...
  <div class="car-table">
    ...
    <el-table
      ...
      <el-table-column
        ...
        header-align="center">
        <template slot-scope="scope">
          <span :class="{oncar:scope.row.is_online==1,offcar:scope.row.is_online==0}">{{ getCarStatus(scope.row.is_online) }}</span>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  methods: {
    setTableHeight(sence){ // sence：1表示地图场景，2表示窗口打开场景
      if(sence === 1){
        this.tableHeight = window.innerHeight + window.screenY - (85+20+40+34+32)
        this.pageSize = 15
      }else if(sence === 2){
    ...
    _getCarList() {
      ...
      getCarList(params).then(res => {
        ...
        if(ret == 0){
          ...
          this.currentPage = datas.cur_page
          this.pageSize = datas.page_size
          this.total = datas.item_counts
        }else{
          this.$message({
            message: msg,
            type: 'error'
