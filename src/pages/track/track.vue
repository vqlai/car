...
<template>
    ...
    <el-container>
      <el-main>
        ...
        <!-- <div id="bdmap"></div> -->
        <bdmap :bmap.sync="baiduMap"></bdmap>
      </el-main>
...
import {drawMapLine} from '@/common/js/bdmap'
export default {
  name: 'car-track',
  data () {
    return {
  ...
  methods:{
      ...
      if (this.trackCount > 1){ // trackCount 轨迹点数量
        ...
        if (this.carMarker != null){
          ...
          for (var i = 1; i < this.trackCount - 1; i++){
            ...
            this.baiduMap.addOverlay(this.carMarker);
        }
        ...
        function resetMkPoint(pIndex){
          if (that.travelIsPlay){
            ...
            // if (document.getElementById("chkTerCenter").checked){
                // that.baiduMap.setCenter(mPoint);
            // }else{//判断是否超出地图范围，超出则设定当前点为中心点
                var bs = that.baiduMap.getBounds();   //获取可视区域
    ...
    _stopOrEndPlay(tag){
      ...
      if (tag == 0 && this.carMarker != null){//主动停止时汽车回到到原点
        ...
        this.baiduMap.setCenter(this.trackMarkers[0].getPosition());
      }
    },
    // 播放轨迹
    handlePlay() {
