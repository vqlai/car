...
export default {
  ...
  methods:{
    ...
    onReturnCarClick(row){
      console.log(row)
      getReturnCarInfo({car_rental_id: row.car_rental_id}).then((res)=>{
        console.log(res)
        if(res.data.ret == 0){
    ...
    comfirmReturnClick(){
      comfirmReturnCar({car_rental_id: this.returnCarInfo.car_rental_id}).then((res)=>{
    ...
    onCurrentChange(page){
      if(!this.pageFlag){
        this.pageFlag = true
        this._getRental({page: page, page_size: this.pageSize})
      }
    },
