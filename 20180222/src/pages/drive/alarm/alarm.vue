...
export default {
  ...
  mounted() {
  	this.$nextTick(()=>{
  	  this._getAlarmInfo({page: this.currentPage,page_size: this.pageSize, vehicle_number: this.queryText})
  	})
  },
  methods:{
  	_getAlarmInfo(params){
  	...
  	onCurrentChange(page){
  		if(!this.pageFlag){
  		  this.pageFlag = true
  		  this._getAlarmInfo({page: page, page_size: this.pageSize, vehicle_number: this.queryText})
  		}
  	},
  	onSizeChange(size){
  		this._getAlarmInfo({page: this.currentPage, page_size: size, vehicle_number: this.queryText})
  	},
  	onSearchClick(){
      ...
      if(res.data.ret == 0){
        this._getAlarmInfo({ page: this.currentPage, page_size: this.pageSize, scheme_name: this.queryText })
