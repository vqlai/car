...
export default {
  ...
  mounted() {
    ...
    if(this.$route.params.id){
      this.carNumber = this.$route.params.id
    }
    let params = {
      page: this.currentPage,
      page_size: this.pageSize,
      vechile_number: this.carNumber
    }
  ...
  methods:{
    ...
    onSizeChange(page){ 
      console.log(page)
      this.pageSize = page 
      this._getDriveRecord({page: this.currentPage, page_size: this.pageSize, vechile_number: this.carNumber})
    },
    // 当前页数变化事件
    onCurrentChange(page){
      ...
      if(!this.pageFlag){
        this.pageFlag = true
        this._getDriveRecord({page: page, page_size: this.pageSize, vechile_number: this.carNumber})
      }
    },
    // 格式化时间
