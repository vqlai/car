        ...
        <el-aside width="320px">
          <el-tabs type="border-card" @tab-click="onTabClick" value="fence">
            <el-tab-pane label="围栏列表" name="fence">
              <fenceTable @onFenceTableCellClick="onFenceTableCellClick" :tableData="fenceData"></fenceTable>
            </el-tab-pane>
            <el-tab-pane label="行政区域" name="area">
              <areaTree @onTreeNodeClick="onTreeNodeClick"></areaTree>
            </el-tab-pane>
            <el-tab-pane label="车辆列表" name="device">
              <deviceTable v-if="deviceTable"></deviceTable>
            </el-tab-pane>
          </el-tabs>
        </el-aside>
      </el-container>
...
<script>
import {addMapOverlay, getBound} from '@/common/js/bdmap'
...
export default {
  ...
  methods:{
    ...
    _getFence() {
      this.fenceData = []
      getFence({monitored_person: '粤B78954'}).then((res)=>{
    ...
    onDialogComfirm() {
      ...
      if(this.dialogForm.type === 1){
        ...
        createPolygonFence(params).then((res)=>{
          this.createFenceRes(res)
        })
      }else{
        return false
